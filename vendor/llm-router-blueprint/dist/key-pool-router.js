 ...
 * a safety net.
 */
export function resolveModelChain(cfg, opts = {}) {
    const strip = (m) => m.replace(/^models\//, '').trim();
...
export function createKeyPoolRouter(opts = {}) {
    ...
    const initPromise = store.load()
        ...
        for (const k of s.deadKeys ?? [])
            deadKeys.add(k);
        for (const m of s.deadModels ?? [])
            deadModels.add(m);
    })
        .catch((err) => log(`[router] state load failed, starting empty: ${String(err)}`));
    const snapshot = () => ({
    ...
    const pairId = (m, k) => `${m}::${k}`;
    const stateOf = (m, k) => quotaState.get(pairId(m, k)) ?? { cooldownUntil: 0, exhaustedOnPtDate: null };
    const release = (pair) => {
    ...
    const pickLocked = (chain, apiKeys) => {
        ...
        const modelOverloaded = (m) => (overloadedModels.get(m) ?? 0) > now();
    ...
    const waitLocked = (chain, apiKeys) => {
        ...
        if (earliest === Infinity && !hasInFlight)
            return null;
        if (earliest !== Infinity && earliest - now() > rpmCooldownMs && !hasInFlight)
            return null;
        return {
    ...
    async function call(chain, apiKeys, exec) {
        await initPromise;
        if (chain.length === 0)
            throw new Error('key-pool router: empty model chain');
        if (apiKeys.length === 0)
                ...
                for (let attempt = 0; attempt < maxAttempts; attempt++) {
                    try {
                        ...
                        await withMutex(async () => {
                            ...
                            if (s)
                                quotaState.set(pair, { cooldownUntil: 0, exhaustedOnPtDate: s.exhaustedOnPtDate });
 ...
 */
export async function callWithKeyPool(chain, apiKeys, exec, opts = {}) {
